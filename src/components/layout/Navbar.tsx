    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 flex items-center justify-between">
          ...
          >
            <span className="font-serif text-2xl sm:text-[26px] tracking-[0.22em] text-[#fcfbf7] font-normal uppercase group-hover:text-[#c8a97e] transition-colors duration-300 leading-none">
              {SITE_CONFIG.brandName}
            </span>
            <span className="text-[8px] sm:text-[9px] tracking-[0.35em] text-[#c8a97e] uppercase font-light mt-1">
          ...
          <div className="flex items-center space-x-3 lg:hidden">
            <Link
              href="#contact"
              className="px-3.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-[#08090d] bg-gold-gradient rounded-sm shadow-md"
            >
              Book Date
            </Link>
            ...
            <button
              ...
              className="p-2 text-[#fcfbf7] hover:text-[#c8a97e] transition-colors focus:outline-none cursor-pointer flex flex-col justify-center items-center gap-1.5 w-9 h-9 rounded-sm bg-white/5"
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 bg-[#08080a]/98 backdrop-blur-2xl flex flex-col justify-between pt-6 pb-8 px-6 lg:hidden animate-fadeIn overflow-y-auto">
          {/* Mobile Header Bar */}
          <div className="flex items-center justify-between pb-5 border-b border-white/10">
            <div>
              <span className="font-serif text-xl tracking-[0.22em] text-[#fcfbf7] uppercase font-medium">
                {SITE_CONFIG.brandName}
              </span>
            ...
            <button
              ...
              className="p-2 rounded-full bg-white/5 text-[#fcfbf7] hover:text-[#c8a97e] border border-white/10 transition-colors cursor-pointer"
          ...
          <nav className="flex flex-col space-y-4 my-8 text-center">
            {navLinks.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                onClick={() => setMobileMenuOpen(false)}
                className="font-serif text-2xl text-[#fcfbf7]/90 hover:text-[#c8a97e] active:text-[#c8a97e] transition-colors py-1.5 tracking-wider"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="p-4 rounded-sm bg-[#121217] border border-[#c8a97e]/25 flex items-center space-x-3 mb-4">
            ...
            <div className="flex-1 min-w-0 text-left">
              <p className="font-serif text-sm text-[#fcfbf7]">{SITE_CONFIG.proprietor}</p>
              <p className="text-[10px] text-[#c8a97e] uppercase tracking-wider">{SITE_CONFIG.proprietorRole}</p>
              <a href={getTelLink()} className="text-[11px] text-[#a39e94] font-mono hover:text-white transition-colors">
                {SITE_CONFIG.phone}
              </a>
            </div>
            <a
              ...
              className="p-2.5 rounded-full bg-[#25D366] text-black shrink-0 hover:scale-105 transition-transform"
          ...
          <div className="space-y-3 pt-2 text-center">
            <Link
              href="#contact"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full flex items-center justify-center gap-2 py-3.5 bg-gold-gradient hover:bg-gold-gradient-hover text-[#08090d] font-bold uppercase tracking-widest text-xs rounded-sm shadow-xl cursor-pointer transition-colors"
            >
